...
const head=(title,desc="")=>`<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(desc)}"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet"><link rel="stylesheet" href="/style.css"></head><body><header class="site-header"><div class="header-inner"><a class="brand" href="/">MaggisRezepte</a><label class="search-box"><span>⌕</span><input id="search" type="search" placeholder="Rezept suchen…"></label></div></header>`;
const card=r=>`<a class="recipe-card" href="/rezept-${r._slug}.html" data-search="${esc(`${r.title} ${r.category} ${r.subcategory}`.toLowerCase())}"><div class="thumb" style="background-image:url('${esc(r.image||"/images/rezept-demo.png")}')"></div><div class="recipe-info"><small>${esc(r.category.toUpperCase())} · ${esc(r.subcategory.toUpperCase())}</small><h3>${esc(r.title)}</h3><p>${esc(r.time)} · ${esc(r.servings)}</p></div><span class="arrow">→</span></a>`;
...
for(const r of recipes){
 const jsonld={"@context":"https://schema.org","@type":"Recipe","name":r.title,"image":r.image?[r.image]:undefined,"recipeYield":r.servings,"recipeIngredient":r.ingredients,"recipeInstructions":r.steps.map(x=>({"@type":"HowToStep","text":x}))};
 const page=`${head(`${r.title} – MaggisRezepte`,`${r.title} – einfaches Rezept.`)}<main class="recipe-layout">${sidebar(r.subcategory)}<article class="recipe-paper"><nav class="breadcrumb"><a href="/">Startseite</a><span>›</span><a href="/kategorie-${slug(r.subcategory)}.html">${esc(r.category)}</a><span>›</span><a href="/kategorie-${slug(r.subcategory)}.html">${esc(r.subcategory)}</a></nav><h1>${esc(r.title)}</h1><div class="meta-row"><span>◷ ${esc(r.time)}</span><span>♙ ${esc(r.servings)}</span><span>▥ ${esc(r.difficulty)}</span></div>${r.image?`<figure class="recipe-image"><div class="photo-tape"></div><img src="${esc(r.image)}" alt="${esc(r.title)}"></figure>`:""}<section class="recipe-section"><h2><span>Zutaten</span></h2><ul>${(r.ingredients||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ul></section><section class="recipe-section"><h2><span>Zubereitung</span></h2><ol>${(r.steps||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ol></section>${r.notes?`<section class="recipe-section"><h2><span>Notizen</span></h2><p>${esc(r.notes)}</p></section>`:""}<button class="print-button" onclick="window.print()">▣ &nbsp; Rezept drucken</button></article></main><script type="application/ld+json">${JSON.stringify(jsonld)}</script></body></html>`;
 fs.writeFileSync(path.join(OUT,`rezept-${r._slug}.html`),page);
}
for(const [cat,subs] of Object.entries(subcats))for(const sub of subs){
 const found=recipes.filter(r=>r.subcategory===sub);
 const p=`${head(`${sub} – MaggisRezepte`,`${sub} Rezepte`)}<main class="recipe-layout">${sidebar(sub)}<section class="home-paper category-page"><div class="section-heading home-heading"><span>${esc(cat.toUpperCase())}</span><h1>${esc(sub)}</h1></div><div id="recipe-list" class="recipe-list home-recipe-list">${found.length?found.map(card).join(""):`<p>Hier gibt es noch keine Rezepte.</p>`}</div></section></main><script src="/script.js"></script></body></html>`;
