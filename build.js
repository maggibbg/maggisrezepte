...
const card=r=>`<a class="recipe-card" href="/rezept-${r._slug}.html" data-search="${esc(`${r.title} ${r.category} ${r.subcategory}`.toLowerCase())}"><div class="thumb" style="background-image:url('${esc(r.image||"/images/rezept-demo.png")}')"></div><div class="recipe-info"><small>${esc(r.category.toUpperCase())} · ${esc(r.subcategory.toUpperCase())}</small><h3>${esc(r.title)}</h3><p>${esc(totalTime(r))} · ${esc(r.servings)}</p></div><span class="arrow">→</span></a>`;
...
for(const r of recipes){
 ...
 const page=`${head(`${r.title} – MaggisRezepte`,r.seo_description||r.description||`${r.title} – einfaches Rezept.`)}<main class="recipe-layout">${sidebar(r.subcategory)}<article class="recipe-paper"><nav class="breadcrumb"><a href="/">Startseite</a><span>›</span><a href="/kategorie-${slug(r.subcategory)}.html">${esc(r.category)}</a><span>›</span><a href="/kategorie-${slug(r.subcategory)}.html">${esc(r.subcategory)}</a></nav><h1>${esc(r.title)}</h1><div class="meta-row"><span>◷ ${esc(totalTime(r))}</span><span>♙ ${esc(r.servings)}</span><span>▥ ${esc(r.difficulty)}</span></div>${r.description?`<p class="recipe-intro">${esc(r.description)}</p>`:""}${(prepTime(r)||cookTime(r))?`<div class="time-detail">${prepTime(r)?`<span>Vorbereitung: <strong>${esc(prepTime(r))}</strong></span>`:""}${cookTime(r)?`<span>Koch-/Backzeit: <strong>${esc(cookTime(r))}</strong></span>`:""}</div>`:""}${r.image?`<figure class="recipe-image"><div class="photo-tape"></div><img src="${esc(r.image)}" alt="${esc(r.image_alt||r.title)}"></figure>`:""}<section class="recipe-section"><h2><span>Zutaten</span></h2><ul>${(r.ingredients||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ul></section><section class="recipe-section"><h2><span>Zubereitung</span></h2><ol>${(r.steps||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ol></section>${r.notes?`<section class="recipe-section"><h2><span>Notizen</span></h2><p>${esc(r.notes)}</p></section>`:""}<button class="print-button" onclick="window.print()">▣ &nbsp; Rezept drucken</button></article></main><script type="application/ld+json">${JSON.stringify(jsonld)}</script></body></html>`;
