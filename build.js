...
const head=(title,desc="",canonical="/")=>`<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(desc)}"><link rel="canonical" href="${SITE}${canonical}"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(desc)}"><meta property="og:url" content="${SITE}${canonical}"><meta property="og:type" content="website"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet"><link rel="stylesheet" href="/style-v19.css"></head><body><header class="site-header"><div class="header-inner"><a class="brand" href="/">MaggisRezepte</a><div class="search-wrap"><label class="search-box"><span>⌕</span><input id="search" type="search" autocomplete="off" placeholder="Rezept suchen…" aria-label="Rezepte durchsuchen"></label><div id="search-results" class="search-results" hidden></div></div></div></header>`;
const card=r=>`<a class="recipe-card" href="/${r._slug}/" data-search="${esc(`${r.title} ${r.category} ${r.subcategory}`.toLowerCase())}"><div class="thumb" style="background-image:url('${esc(recipeImage(r))}')"></div><div class="recipe-info"><h3>${esc(r.title)}</h3><p>${esc(totalTime(r))} · ${esc(r.servings)}</p></div><span class="arrow">→</span></a>`;
const catCards=()=>`<div class="home-category-grid">${Object.entries(subcats).map(([cat,subs])=>`<article class="home-category-card"><div class="tape"></div><h2>${cat}</h2><nav>${subs.map(s=>`<a href="/kategorie-${slug(s)}.html">${esc(s)} <span>›</span></a>`).join("")}</nav></article>`).join("")}</div>`;
const home=`${head("MaggisRezepte – Mein digitales Rezept-Notizbuch","Einfache Rezepte zum Kochen und Backen.","/")}<main class="home-layout">${sidebar()}<section class="home-paper"><div class="section-heading home-heading"><h1>Kategorien</h1></div>${catCards()}<div class="paper-divider"></div><div class="section-heading"><h2>Neue Rezepte</h2></div><div id="recipe-list" class="recipe-list home-recipe-list">${recipes.slice(0,3).map(card).join("")}</div></section></main><footer class="site-footer"><div><strong>MaggisRezepte</strong><span>Mein digitales Rezept-Notizbuch</span></div><nav><a href="#">Datenschutz</a><a href="#">Impressum</a></nav></footer><script src="/script.js"></script></body></html>`;
